...
describe('scanner', () => {
  ...
  describe('scanLine()', () => {
    ...
    it('detects Stripe secret key', () => {
      const line = 'STRIPE_SECRET_KEY=sk' + '_live_XXXXXXXXXXXXXXXXXXXXXXXXXX';
    ...
    it('detects Stripe test key with medium severity', () => {
      const line = 'STRIPE_TEST_KEY=sk' + '_test_XXXXXXXXXXXXXXXXXXXXXXXX';
      const findings = scanLine(line, 1, '.env');
    ...
    it('detects Slack webhook URL', () => {
      const line = 'SLACK_WEBHOOK=https://hooks.slack.com/services/T000000000/B000000000/AAAAAAAAAAAAAAAAAAArStUvWx';
      const findings = scanLine(line, 1, '.env');
  ...
  describe('scanFile()', () => {
    it('detects secrets in a file', () => {
      const fp = writeFixture('secrets.env', [
        ...
        'STRIPE_SECRET=sk' + '_live_XXXXXXXXXXXXXXXXXXXXXXXX'
      ].join('\n'));
